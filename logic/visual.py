        ...
        while True:
            ...
            if cfg.show_window:
                if hotkeys_watcher.filter_own_player_enabled:
                    filter_status = f'Own Player Filter: ON ({hotkeys_watcher.filter_own_player_side.upper()})'
                else:
                    filter_status = 'Own Player Filter: OFF'
                cv2.putText(self.image, filter_status, (10, y_pos), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 255), 1, cv2.LINE_AA)
